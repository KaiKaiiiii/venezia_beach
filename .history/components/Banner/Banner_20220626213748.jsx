...
const Banner = () => {
  return (
    <section className="w-full h-[500px] relative leading-3">
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 bottom-0 translate-y-1/2 z-[6] w-full p-[40px]">
        <div className="container py-4 px-8 w-[340px] h-[370px] rounded-lg blurry">
...
const Hightlight = ({ percent, title, content, ...props }) => {
  return (
    <div
      className={`flex flex-col items-center justify-start gap-4 text-center ${props} w-full h-full `}
    >
      <h2 className="bg-text-warning text-secondary  bg-clip-text fill-transparent font-bold text-2xl mb-2 leading-3">
        {percent}
      </h2>
      <p className="text-primary text-xl font-semibold">{title}</p>
      <p className="text-gray text-xs">{content}</p>
    </div>
  );
};
