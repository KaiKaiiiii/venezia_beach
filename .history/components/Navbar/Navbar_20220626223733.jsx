  ...
  return (
    <header className="top-0 left-0 right-0 fixed z-50">
      <div className="container px-2 px-sm-4">
        <nav className=" top-0 shadow mt-0 mt-sm-3 py-2  rounded-b-2xl blurry xl:rounded-full xl:mt-5 xl:bg-opacity-50 ">
                ...
                {navBarLinks.map((link) => {
                  return (
                    <li key={link.name}>
                      <a
                        href={link.href}
                        className="uppercase font-medium text-primary block"
                      >
                        {link.name}
                      </a>
                    </li>
